const TerrainUtil =
{
     ...
     */
    growTerr( map, mapWidth, mapHeight, type, optChanceThreshhold ) {
        ...
        for ( x = 0, y = 0; y < mapHeight; x = ( ++x === mapWidth ? ( x % mapWidth + ( ++y & 0 ) ) : x ) )
        {
            ...
            if ( map[ index ] === type ) {
                const pi = TerrainUtil.getSurroundingIndicesFor( x, y, mapWidth, mapHeight, Math.random() > .7 );
     ...
     */
    getSurroundingIndicesFor( x, y, mapWidth, mapHeight, inclDiagonals ) {
     ...
     */
    positionAtRandomFreeTileType( terrain, tileType ) {
        let tries = 255, i;
